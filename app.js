...
io.on('connection', function (socket) {
    ...
    socket.on('get_best', function () {
        var username = socket.request.sess.username;
        db.getBest(username,function (score) {
            if (score.length >0) 
            {
                console.log("score[0].score "+score[0].score )
                socket.emit('send_best', score[0].score);    
            }
            else
            {
                socket.emit('send_best', 0);    
            }
